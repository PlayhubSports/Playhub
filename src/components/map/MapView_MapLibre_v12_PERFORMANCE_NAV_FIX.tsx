...
// PlayHub MapLibre v12 — mapa real vetorial + otimização de performance/navegação.
...
function buildPlayHubVectorStyle(rawStyle: PlayHubStyle): PlayHubStyle {
  const nextStyle: PlayHubStyle = {
    ...rawStyle,
    name: 'PlayHub Dark Pulse Vector v12',
    metadata: {
      ...(rawStyle.metadata ?? {}),
      'playhub:purpose': 'Mapa esportivo real: cidades/bairros/ruas controladas por zoom, POIs desligados, pins fixos e navegação otimizada.',
    },
    glyphs: rawStyle.glyphs,
    sprite: rawStyle.sprite,
    layers: (rawStyle.layers ?? []).map((layer: PlayHubLayer) => {
      ...
      if (nextLayer.type === 'symbol') {
        if (isRoadLabelLayer(nextLayer)) {
          ...
          nextLayer.paint = {
            ...nextLayer.paint,
            'text-color': 'rgba(178, 211, 218, 0.62)',
        ...
        if (isPlaceLabelLayer(nextLayer)) {
          ...
          nextLayer.paint = {
            ...
            'text-color': [
              'interpolate', ['linear'], ['zoom'],
              7, 'rgba(146, 183, 194, 0.50)',
              12, 'rgba(179, 222, 228, 0.58)',
              15, 'rgba(206, 248, 239, 0.62)',
            ],
      ...
      if (nextLayer.type === 'line') {
        ...
        if (includesAny(signature, ['waterway', 'boundary'])) {
          nextLayer.paint = {
            ...nextLayer.paint,
            'line-color': signature.includes('waterway') ? '#07383a' : '#103135',
            'line-opacity': signature.includes('waterway') ? 0.12 : 0.08,
            'line-width': signature.includes('waterway') ? 0.7 : 0.45,
          }
          return nextLayer
        }
      ...
      if (nextLayer.type === 'fill') {
        ...
        if (signature.includes('building')) {
          nextLayer.minzoom = Math.max(Number(nextLayer.minzoom ?? 0), 15.1)
          nextLayer.paint = {
            ...nextLayer.paint,
            'fill-color': '#061011',
            'fill-opacity': [
      ...
      position => {
        ...
        if (centerMap && mapRef.current) {
          mapRef.current.flyTo({
            ...
            zoom: 14.2,
            duration: getPlayHubMotionDuration(360),
            essential: true,
          })
        }
      },
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      try {
        ...
        const styleResponse = await fetch(PLAYHUB_MAP_STYLE_URL)

        if (!styleResponse.ok) {
          throw new Error(`Style load failed: ${styleResponse.status}`)
        }

        const rawStyle = await styleResponse.json()
        const playHubStyle = buildPlayHubVectorStyle(rawStyle)
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="relative flex-1 overflow-hidden bg-ph-dark2">
        ...
        <button
          ...
          className="absolute top-2.5 right-2.5 z-[11] flex items-center gap-1.5 rounded-[14px] px-3 py-2 text-[11px] font-extrabold text-ph-text backdrop-blur-md"
          style={{
        ...
        {visibleArenas.length === 0 && !mapLoading && (
          ...
          >
            ...
            <p className="text-[12px] text-ph-muted mt-1">
              Ainda não há instituições cadastradas para este filtro.
            </p>
          </div>
        )}
